...
def solucion(A):
    ...
    n_cuadrados_perfectos = 0 # cuantos cuadrados perfectos hay
    cuadrados_perfectos = [] #lista de cuales cuadrados perfectos son
    for i in range (len(A)): #longitud de filas
        for j in range (len(A[0])): #longitud de colomnas en pocision inicial
            if float.is_integer(sqrt(A[i][j])): ##
                n_cuadrados_perfectos += 1  #para sumar la cantidad de cuadrados
                cuadrados_perfectos.append(A[i][j])  #añadir a la lista los numeros que son cuadrados perfectos
